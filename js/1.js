function setCanvas(obj, className){
    ...
    } else if (obj.typeCanvas === 1 || obj.typeCanvas === 2) {
        canvas.style['left'] = (obj.x - obj.width/2) + (obj.offsetX || 0) + 'px';
        canvas.style['top'] = (obj.y - obj.height) + (obj.offsetY || 0) + 'px';
    }
...
function getAbsoluteCanvasPos(pos, obj){
    var x, y, offsetCanvasX, offsetCanvasY;
    if (obj.typeCanvas === 0){
        offsetCanvasX = obj.x - obj.canvas.width/2;
        offsetCanvasY = obj.y - obj.canvas.height/2;
        x = pos.x + offsetCanvasX; 
        y = pos.y + offsetCanvasY;
    } else if (obj.typeCanvas === 1 || obj.typeCanvas === 2){
        offsetCanvasX = obj.x - obj.canvas.width/2;
...
function updateCanvas(obj){
    if (obj.old_angle !== null){
        ...
        if (da !== 0){
            ...
            } else if (obj.typeCanvas === 2){
                doRotate(obj, obj.angle, (100 + obj.offsetX) + '% ' + (100 + obj.offsetY) + '% 0');
            }
        }
    }
...
function removeAllClass(idParent, className){
    var parent = document.getElementById(idParent);
    var classes = parent.getElementsByClassName(className);
    for(var i = classes.length - 1; i >= 0; i--){
        parent.removeChild(classes[i]);
    }
}